      ...
      <div className='grid grid-cols-4 w-full max-xl:grid-cols-3 max-sm:grid-cols-2 gap-x-6'>
        ...
        {items.map((movie, index) => (
          <div className={`rounded-lg w-full mb-8 duration-500 ${searchQuery ? '' : 'opacity-100'}`} key={movie.title}>
            ...
            <div className={`flex justify-center items-center rounded-lg bg-cover w-full lg:h-[174px] max-lg:h-[150px] cursor-pointer relative  duration-500 ease-out`} style={{ backgroundImage: `url(${movie.image})` }}>
            ...
            <ul className='flex items-center gap-2 sm:text-sm max-sm:text-xs text-greyish-blue mt-1 select-none max-h-5'>
              ...
              <li>
                <div className='flex items-center gap-1'>
                  ...
                  <img src={`${movie.category === "Movie" ? '/images/icon-category-movie.svg' : '/images/icon-category-movie.svg'}`} />
                  <p>{movie.category}</p>
                </div>
              </li>
              <li className='flex items-center'><span className='bullet bg-greyish-blue' />{movie.rating}</li>
            </ul>
