...
const mappedSeries = series.filter(item => selectedMovies.includes(item.title))
  ...
  const filteredSeries = mappedSeries.filter((movie) =>
    movie.title.toLowerCase().includes(searchQuery.toLowerCase())
  );
            ...
            {filteredMovies.map((movie, index) => (
              <div className={`rounded-lg w-full mb-8 duration-500 ${searchQuery ? '' : 'opacity-100'}`} key={movie.title}>
                ...
                <div className='flex justify-center items-center rounded-lg bg-cover w-full lg:h-[174px] max-lg:h-[150px] cursor-pointer relative  duration-500 ease-out' style={{ backgroundImage: `url(${movie.imageLarge})` }} >
                  ...
                  <div id='bookmark' className={`rounded-full bg-black hover:bg-white hover:bg-opacity-80 bg-opacity-40 w-8 h-8 absolute cursor-pointer right-3 top-3 flex items-center justify-center hover:stroke-black stroke-white fill-transparent duration-300 ease-in ${selectedMovies.includes(movie.title) ? 'fill-white hover:fill-transparent hover:stroke-white hover:bg-black' : ''
                    }`}
